...
扩展必须更新到 WeKnora 配套 0.3.0 构建。官方 0.3.0 原版缺少预览、定位和释放控制接口，不能直接替换配套 ZIP。升级前结束任务；在原解压目录覆盖文件后重新加载扩展，可沿用原扩展 ID 并迁移授权凭据。重新安装导致 ID 改变时重新配对。服务端必须同步升级 bsk：从与扩展相同的固定 0.3.0 源码构建，然后重启后端以释放旧 daemon。官方 CLI 0.2.1 不支持当前滚动等工具方法。
