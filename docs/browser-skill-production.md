...
扩展必须更新到 WeKnora 配套 0.3.0 构建。官方 0.3.0 原版缺少预览和定位接口，不能直接替换配套 ZIP。升级前结束任务；在原解压目录覆盖文件后重新加载扩展，可沿用原扩展 ID 并迁移授权凭据。重新安装导致 ID 改变时重新配对。服务端必须同步升级 bsk：从与扩展相同的固定源码提交 `fa953dc6fcd868827b93164e3bea26198e691224` 构建（版本号仍为 0.3.0，需重新构建并更新扩展），然后重启后端以释放旧 daemon。官方 CLI 0.3.0 不包含配套 daemon 的导航响应宽限补丁。
