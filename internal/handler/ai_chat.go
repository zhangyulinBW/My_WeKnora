...
func (h *AIChatHandler) Chat(c *gin.Context) {
	...
	chatModel, err := h.modelService.GetChatModel(ctx, agent.Config.ModelID)
	if err != nil {
...
// endpoint. The agent id is configurable via WEKNORA_AI_AGENT_ID.
func (h *AIChatHandler) resolveAgent(ctx context.Context) (*types.CustomAgent, error) {
	id := strings.TrimSpace(os.Getenv("WEKNORA_AI_AGENT_ID"))
	if id == "" {
		id = defaultAIAgentID
	}
...
func (h *AIChatHandler) classifyIntent(
	...
	sys := "你是意图分类器。判断用户消息是要执行表格搜索/筛选查询，还是普通问答/页面介绍。只输出一个词：search 或 normal。"
	user := "用户消息：" + message
...
func (h *AIChatHandler) handleNormalAnswer(
	...
	userContent := buildNormalUserPrompt(req.Message, req.Page, lang)
...
func buildNormalUserPrompt(message string, page *AIPageContext, lang string) string {
	var sb strings.Builder
	if page != nil {
		pageJSON, _ := json.Marshal(page)
		sb.WriteString("[用户提供的结构化上下文]\n")
		sb.WriteString(string(pageJSON))
		sb.WriteString("\n\n")
	}
