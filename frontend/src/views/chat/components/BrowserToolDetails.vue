<template>
  <section class="browser-tool-details">
    <p class="browser-result-status" :class="{ failed: event.success === false }">{{ browserToolSummary(t, event) }}</p>
...
import { browserToolContent, browserToolSummary, type BrowserToolEvent } from '@/utils/browserToolDisplay'
...
.browser-result-status.failed { color: var(--td-error-color); }
