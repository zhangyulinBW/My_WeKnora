...
不再需要的文件可以在会话文件面板或「产物」页删除。删除会回收存储中的字节，无法恢复，因此会先确认；只有会话归属人能删除，通过共享智能体获得的只读访问只能下载。会话文件面板按每次生成分别列出，删除的是点击的那一版；「产物」页一行代表一个文件（带版本数），删除会连同该文件在这个会话中的所有历史版本。同一份文件如果已被存入知识库或被后续回答重新引用，那些位置不受影响。删除后沙箱里的同名文件也不会被重新收录。
