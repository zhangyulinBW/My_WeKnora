...
### 日志系统（`internal/logger`） {#_2-日志系统-internal-logger}
...
#### LLM 调试日志（`internal/logger/llm_logger.go`） {#_2-3-llm-调试日志-internal-logger-llm-logger-go}
...
#### 请求日志中间件（`internal/middleware/logger.go`） {#_2-4-请求日志中间件-internal-middleware-logger-go}
...
#### 配置（环境变量，`config.go`） {#_3-1-配置-环境变量-config-go}
...
#### 数据模型（`internal/types/audit_log.go`） {#_4-1-数据模型-internal-types-audit-log-go}
...
#### 写入路径（service + middleware） {#_4-3-写入路径-service-middleware}
...
#### 通用滑动窗口限流器（`internal/ratelimit/limiter.go`） {#_5-1-通用滑动窗口限流器-internal-ratelimit-limiter-go}
...
#### 公开认证端点 IP 限流（`internal/middleware/auth_public_ratelimit.go`） {#_5-2-公开认证端点-ip-限流-internal-middleware-auth-public-ratelimit-go}
...
### 模型引用统计（`internal/application/repository/model_usage.go`） {#_7-模型引用统计-internal-application-repository-model-usage-go}
